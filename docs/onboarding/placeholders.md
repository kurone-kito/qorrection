...
### `cargo fmt --check && cargo clippy --all-targets -- -D warnings && cargo test --all-targets --all-features`
...
| -------------------------------- | --------------------------------------------------------- | ---------------------------------- |
| `qorrection`                  | Repository short name used in worktree examples           | `my-app`                           |
| `qorrection`      | Hidden issue-body marker prefix                           | `my-app`                           |
| `kurone-kito`       | Single JSON-escaped login allowed to post trusted markers | `trusted-user-a`                   |
| `cargo fmt && cargo clippy --all-targets -- -D warnings`      | Auto-fix plus validate command row                        | `npm run lint:fix && npm run lint` |
| `cargo fmt --check && cargo clippy --all-targets -- -D warnings && cargo test --all-targets --all-features` | Non-mutating verify command row                           | `npm run lint && npm run test`     |
| `cargo fmt --check && cargo clippy --all-targets -- -D warnings && cargo test --all-targets --all-features` | Post-fix validate command row                             | `npm run lint:fix && npm test`     |
...
- no relevant auto-fix command →
  `cargo fmt && cargo clippy --all-targets -- -D warnings = true`
...
`qorrection` appears in two hidden issue-body markers:

- roadmap identity marker:
  `<!-- qorrection-roadmap-id: {unique-id} -->`
- blocked-by marker:
